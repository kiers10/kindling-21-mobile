...
export default class passResetScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      email: global.email,
    ...
    return (
      <View style={styles.container}>
        <ImageBackground source={background} style={styles.background}>
          ...
          <Text style={styles.subtitle}>A password reset code has been sent to:</Text>
          <Text style={styles.emailtitle}>{this.state.email}</Text>
          <Text style={styles.subtitle}>Please type in the code below. Upon success, your password will be changed.</Text>
          <TextInput
            style={styles.input}
            placeholder={"Verification Code"}
            onChangeText={(val) => {this.setVerifyCode(val)}}
          />
          <Text style={styles.tiny} onPress={this.resendVer}>Resend email</Text>
  ...
  handleClick = async() => {
    try {
      var loginInfo = {
        ...
        new_password_str: global.password.trim(),
      }
      var jsonObj = JSON.stringify(loginInfo);

      const response = await fetch('https://kindling-lp.herokuapp.com/api/reset_password', 
